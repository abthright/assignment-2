...
import { CreateTicketSchema } from "@/src/utils/zodSchemas";
...
app
  ...
  .patch("/:id", zValidator("json", CreateTicketSchema), async (c) => {
    const id = c.req.param("id");
    const data = await c.req.valid("json");
    ...
    return c.json({ message: `successfully updated to ${updateTicket.name}` });
  })
  .delete("/:id", async (c) => {
    const id = c.req.param("id");
